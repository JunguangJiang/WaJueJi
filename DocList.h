...
#define weight 10//计算总权重时的参数

class DocNode{//文档链表节点
public:
	int DocID;//出现该单词的文档ID
	int Times;//单词的出现次数
	int n;//该文档出现了多少个关键词,在后续文档链表的合并时使用
...
public:
	...
	~DocList(void);
	DocNodePosi Add(int DocID, int Times);//添加文档,返回插入的文档节点位置.如果原先存在文档，则对应的次数增加Times
	DocNodePosi Add(int DocID){return Add(DocID, 1);}//添加文档(次数为1），返回插入的文档节点的位置.如果原先存在文档，则对应的次数增加1
